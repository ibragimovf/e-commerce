...
public class MerchantController {
    ...
    public String getMerchantList(Model model, @PathVariable Optional<Integer> pageSize, @ModelAttribute MerchantReceiveDto merchant) {
        List<MerchantResponse> merchantList = (List<MerchantResponse>) restTemplate.postForEntity("http://REST-SERVICE/api/merchant/list/" + getPage(pageSize) + "", merchant, ApiResponse.class).getBody().getT();
        List<GatewayMerchantResponse> gatewayMerchantList = (List<GatewayMerchantResponse>) restTemplate.getForEntity("http://REST-SERVICE/api/gateway/merchant/list/all", ApiResponse.class).getBody().getT();
    ...
    public String getMerchant(Model model, @PathVariable long id) {
        Object t = restTemplate.getForEntity("http://REST-SERVICE/api/merchant/get/" + id + "", ApiResponse.class).getBody().getT();
        MerchantResponse merchantResponse = objectMapper.convertValue(t, MerchantResponse.class);
        List<GatewayMerchantResponse> gatewayMerchantResponseList = (List<GatewayMerchantResponse>) restTemplate.getForEntity("http://REST-SERVICE/api/gateway/merchant/list/all", ApiResponse.class).getBody().getT();
