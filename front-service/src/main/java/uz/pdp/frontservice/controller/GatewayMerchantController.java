...
public class GatewayMerchantController {
    ...
    public String getGatewayMerchantList(Model model, @PathVariable Optional<Integer> pageSize, @ModelAttribute GatewayMerchantReceiveDto gatewayMerchant) {
        List<GatewayMerchantResponse> gatewayMerchantList = (List<GatewayMerchantResponse>) restTemplate.postForEntity("http://REST-SERVICE/api/gateway/merchant/list/" + getPage(pageSize) + "", gatewayMerchant, ApiResponse.class).getBody().getT();
        List<GatewayResponse> gatewayList = (List<GatewayResponse>) restTemplate.getForEntity("http://REST-SERVICE/api/gateway/list/all", ApiResponse.class).getBody().getT();
    ...
    public String getGatewayMerchant(Model model, @PathVariable long id) {
        Object t = restTemplate.getForEntity("http://REST-SERVICE/api/gateway/merchant/get/" + id + "", ApiResponse.class).getBody().getT();
